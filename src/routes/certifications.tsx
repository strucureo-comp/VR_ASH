...
export const Route = createFileRoute("/certifications")({
  head: () => ({
    meta: [
      { title: "Certifications & Manufacturing | Vallalaar Remedies" },
      {
        name: "description",
        content:
...
const DOCS = [
  { title: "WHO / GMP", body: "Manufacturing and quality-related certification documentation." },
  { title: "AYUSH", body: "Ayurvedic regulatory information." },
  { title: "ISO 9001:2015", body: "Quality management certification information." },
];
    ...
    <>
      <section className="bg-[color:var(--surface)] px-6 py-20">
        <div className="mx-auto max-w-4xl">
          <p className="eyebrow text-[color:var(--gold)]">09 &nbsp;·&nbsp; Manufacturing</p>
          ...
          {DOCS.map((d, i) => (
            <Reveal key={d.title} delay={i * 0.06}>
              <article className="h-full rounded-lg border border-border bg-card p-7">
                <FileText className="h-6 w-6 text-[color:var(--botanical)]" />
                ...
                <p className="mt-6 text-xs text-[color:var(--burgundy)]">View / Download Documents</p>
              </article>
            </Reveal>
          ))}
        </div>
